...
| Territory | games/territory/ | Qix/Xonix/JezzBall territory-capture on a radial wheel map — 24 spokes (12 pure + 12 mixed) x 6 rings = 144 levels built from 18 mechanics (Classic, Swarm, Sparx, Qix, Gravity, Splitter, Fuse, Hunter, Ice, Wind, Mirror, Boss + Maze, Shrink, JezzBall, Portal, Bomber, Ghost); 4-directional touch joypad, true Xonix claiming with animated fills, mixed spokes stack up to 5 mechanics by ring, ring difficulty ramp, Easy/Normal/Hard, per-profile per-difficulty progress |
| Air Hockey World Cup | games/airhockey/ | Air-hockey knockout tournament — pick 1 of 16 nations and win the Round of 16 → Quarter-final → Semi-final → Final (first to 7 goals) to lift the World Cup. Portrait rink, drag-your-mallet touch control, skill-rated CPU that ramps by round, Caleb/Ezra profiles. Optional **Super Cup** mode gives each nation a signature special move (Power Slam, Big Wall, Deep Freeze, Goal Shield, Magnet, Slow-Mo) charged by a meter. Multi-file ES modules under `games/airhockey/js/`. |
